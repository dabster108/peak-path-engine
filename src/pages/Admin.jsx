...
export default function Admin() {
  ...
  // ── Load admin chat sessions ─────────────────────────────
  useEffect(() => {
    if (tab !== "chat") return;

    let cancelled = false;

    const loadAdminChats = async ({ silent = false } = {}) => {
      if (!silent && !cancelled) setChatLoading(true);
      try {
        ...
        setSelectedChatSessionId((prev) => {
          ...
          return sorted.some((s) => s.id === prev) ? prev : sorted[0].id;
        });
      } catch {
        if (!silent && !cancelled)
          showToast("Failed to load chat sessions.", "warning");
      } finally {
        if (!silent && !cancelled) setChatLoading(false);
      }
    };

    loadAdminChats();
    const poll = setInterval(() => loadAdminChats({ silent: true }), 12000);
  ...
  useEffect(() => {
    ...
    api
      .get("about/reviews/")
      .then((res) => setAboutFeedbacks(Array.isArray(res.data) ? res.data : []))
      .catch(() => showToast("Failed to load feedback.", "warning"))
  ...
  const totalValue = products.reduce(
    (s, p) => s + parseFloat(p.price) * p.stock,
    0,
  );
  ...
  const addEditProduct = () =>
    setEditForm((prev) => ({
      ...prev,
      products: [...prev.products, buildEditableProduct()],
    }));
  ...
  const saveEdit = async () => {
    ...
    for (const product of editForm.products) {
      if (!product.name.trim()) {
        setEditError("Each product must have a name.");
        return;
      }
      if (!product.category) {
        setEditError("Each product must have a category.");
        return;
      }
      if (!product.section) {
        setEditError("Each product must have a section.");
        return;
      }
      ...
      if (isNaN(price) || price < 0) {
        setEditError("Each product needs a valid price.");
        return;
      }
      if (mrp !== null && isNaN(mrp)) {
        setEditError("Original price must be a valid number.");
        return;
      }
      if (mrp !== null && price > mrp) {
        setEditError("Discounted price cannot exceed original price.");
        return;
      }
      if (isNaN(stock) || stock < 0) {
        setEditError("Each product needs a valid stock quantity.");
        return;
      }
    }
    ...
    try {
      ...
      for (const id of originalEditProductIds.filter(
        ...
        if (product.id) {
          ...
          if (product.imageFiles?.length > 0) {
            for (const img of product.existingImages || []) {
              if (img?.id)
                await api.delete(`products/${product.id}/images/`, {
  ...
  const handleAdd = async () => {
    ...
    if (addForm.stock === "" || isNaN(Number(addForm.stock))) {
      setAddError("Enter a valid stock.");
      return;
    }
  ...
  const filteredFeedbacks = aboutFeedbacks.filter((feedback) => {
    ...
    if (!q) return true;
    return (
      (feedback.name || "").toLowerCase().includes(q) ||
      (feedback.product || "").toLowerCase().includes(q) ||
      (feedback.location || "").toLowerCase().includes(q) ||
      (feedback.text || "").toLowerCase().includes(q)
    );
  });

  // ── Analytics ──────────────────────────────────────────────
  const totalOrdersCount = sortedAdminOrders.length;
  const totalRevenue = sortedAdminOrders.reduce(
    (s, o) => s + Number(o.subtotal || 0),
    0,
  );
  const avgOrderValue = totalOrdersCount ? totalRevenue / totalOrdersCount : 0;
  const deliveredOrders = sortedAdminOrders.filter(
    (o) => o.statusLabel === "Delivered",
  ).length;
  ...
  const ordersByStatus = ORDER_STATUSES.map((status) => {
    const count = sortedAdminOrders.filter(
      (o) => o.statusLabel === status,
    ).length;
    return {
      status,
      count,
      pct: totalOrdersCount ? Math.round((count / totalOrdersCount) * 100) : 0,
    };
  });
  const topOrderedProducts = Object.entries(
    sortedAdminOrders.reduce((acc, order) => {
      (order.items || []).forEach((item) => {
        acc[item.name || "Unknown"] =
          (acc[item.name || "Unknown"] || 0) + Number(item.quantity || 1);
      });
      return acc;
    }, {}),
  )
  ...
  return (
    <div className="admin-layout">
      ...
      <main className="admin-main">
            ...
            {showAdd && (
              <div className="admin-add-form">
                ...
                <div className="admin-add-form__grid">
                  ...
                  {getSubSectionsForSection(addForm.section, sections).length >
                    0 && (
                    <div className="admin-field">
                      <label>Sub-section</label>
                      <select
                        value={addForm.sub_section}
                        onChange={(e) =>
                          setAddForm((f) => ({
                            ...f,
        ...
        {tab === "orders" && (
          <div className="admin-content admin-orders">
            ...
            <div className="admin-filters admin-orders__filters">
              ...
              <div className="admin-filter-chips">
                {["All", ...ORDER_STATUSES].map((s) => (
                  <button
                    key={s}
                    className={`admin-chip${orderStatusFilter === s ? " active" : ""}`}
            ...
            <div className="admin-table-wrap">
              {ordersLoading ? (
                <div className="admin-table__empty">Loading orders…</div>
              ) : (
                <table className="admin-table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    ...
                    {filteredOrders.map((order) => {
                      const currentStatus = ORDER_STATUSES.includes(
                        order.statusLabel,
                      )
                        ? order.statusLabel
                        : "Confirmed";
                        ...
                        <tr key={order.id} className="admin-table__row">
                          ...
                          <td>
                            <select
                              ...
                              value={currentStatus}
                              onChange={(e) =>
                                handleAdminOrderStatusChange(order, e)
                              }
                            >
        ...
        {/* ─── Chat ──────────────────────────────────────── */}
        {tab === "chat" && (
          <div className="admin-content admin-chat">
            <div className="admin-section-header">
              <div>
                <h2 className="admin-section-title">Customer Chat</h2>
                <p className="admin-section-sub">
                  Incoming user messages on the left, live conversation on the
                  right
                </p>
              </div>
            </div>
            ...
            <div className="admin-chat-layout">
              <aside className="admin-chat-sessions">
                <div className="admin-chat-sessions__head">Chat Us Inbox</div>
                {chatLoading ? (
                  <div className="admin-chat-empty">
                    Loading chat sessions...
                  </div>
                ) : chatSessions.length === 0 ? (
                  <div className="admin-chat-empty">No chat messages yet.</div>
                ) : (
                      ...
                      >
                        <div className="admin-chat-session-item__top">
                          <span className="admin-chat-session-item__name">
                            {session.user_username || "User"}
                          </span>
                          {session.unread_count > 0 && (
                            <span className="admin-chat-session-item__unread">
                              {session.unread_count}
                            </span>
                          )}
                        </div>
                        <div className="admin-chat-session-item__preview">
                          {session.last_message || "No message yet"}
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <section className="admin-chat-thread">
                {!selectedChatSession ? (
                  ...
                  <>
                    <div className="admin-chat-thread__head">
                      <div>
                        <h3>{selectedChatSession.user_username || "User"}</h3>
                        <p>{selectedChatSession.user_email || "No email"}</p>
                      </div>
                    </div>

                    <div className="admin-chat-thread__messages">
                      {(selectedChatSession.messages || []).map((message) => (
                        <div
                          key={message.id}
                          className={`admin-chat-bubble-row ${message.sender}`}
                        >
                          <div
                    ...
                    <div className="admin-chat-reply">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (!selectedChatSessionId || !adminReplyText.trim())
                            return;
                          setSendingReply(true);
                          try {
                            await api.post(
                              `admin/chat/${selectedChatSessionId}/`,
                            ...
                            setSelectedChatSessionId((prev) => {
                              if (!sorted.length) return null;
                              return sorted.some((s) => s.id === prev)
                                ? prev
                                : sorted[0].id;
                            });
                            setAdminReplyText("");
                            ...
                            showToast("Failed to send reply.", "warning");
                          } finally {
        ...
        {/* ─── Feedback ──────────────────────────────────── */}
        {tab === "feedback" && (
          <div className="admin-content admin-feedback">
            <div className="admin-section-header">
              <div>
                <h2 className="admin-section-title">Customer Feedback</h2>
                <p className="admin-section-sub">
                  All product feedback submitted from the site
                </p>
              </div>
            </div>
            ...
            <div className="admin-users-toolbar">
              <div className="admin-search-wrap">
                ...
                <input
                  className="admin-search"
                  placeholder="Search by name, product, location, or message"
            ...
            <div className="admin-table-wrap">
              {feedbackLoading ? (
                <div className="admin-table__empty">Loading feedback...</div>
              ) : (
                <table className="admin-table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    ...
                    {filteredFeedbacks.map((feedback) => (
                      <tr key={feedback.id} className="admin-table__row">
                        ...
                        <td className="admin-feedback-rating">
                          {"★".repeat(Number(feedback.rating || 0))}
                          {"☆".repeat(
                            Math.max(0, 5 - Number(feedback.rating || 0)),
                          )}
                        </td>
                        <td className="admin-feedback-text">
                          {feedback.text || "—"}
                        </td>
                        <td className="admin-table__section">
                          {feedback.location || "—"}
      ...
      <Modal
        ...
        actions={
          <>
            <button className="admin-btn-ghost" onClick={closeEditModal}>
              Cancel
            </button>
            <button
              className="admin-btn-primary"
              ...
              {editForm.products.map((product, index) => (
                <div key={product.localId} className="admin-edit-product-card">
                  ...
                  <div className="admin-add-form__grid admin-add-form__grid--edit-modal">
                    ...
                    {getSubSectionsForSection(product.section, sections)
                      .length > 0 && (
                      <div className="admin-field">
                        <label>Sub-section</label>
                        <select
                          value={product.sub_section || ""}
                          onChange={(e) =>
                            updateEditProduct(
                              product.localId,
                              "sub_section",
                    ...
                    <div className="admin-field admin-field--image">
                      ...
                      <div className="admin-image-upload">
                        <input
                          id={`admin-edit-image-${product.localId}`}
                          className="admin-image-input"
                          type="file"
                          accept="image/jpeg,image/png,image/webp"
                          multiple
                          onChange={(e) =>
                            handleEditProductImageSelect(product.localId, e)
                          }
                        />
                        <label
                          className="admin-image-button"
                          htmlFor={`admin-edit-image-${product.localId}`}
                        >
                          Choose Images
                        </label>
                        <span className="admin-image-hint">
                          Selecting new files replaces existing images on save
                        </span>
                      </div>
                      {product.existingImages?.length > 0 &&
                        !product.imagePreviews?.length && (
                          <div className="admin-image-previews">
                          ...
                          {product.imagePreviews.map((src, i) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                className="admin-image-preview__remove"
                                onClick={() =>
                                  removeEditProductNewImage(product.localId, i)
                                }
                              >
                                Remove
                              </button>
