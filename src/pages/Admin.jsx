...
const DEFAULT_CATEGORY_OPTIONS = ["Men", "Women"];
...
const normalizeCategory = (value = "") => {
  ...
  if (raw === "women" || raw === "womens") return "Women";
  return "";
};
...
export default function Admin() {
        ...
        products: prev.products.map((product) =>
          product.localId === localId
            ? {
                ...
                imagePreviews: [...(product.imagePreviews || []), ...previews],
              }
            : product,
        ),
      }));
    ...
    setEditForm((prev) => ({
      ...
      products: prev.products.map((product) =>
        product.localId === localId
          ? {
              ...product,
              imageFiles: (product.imageFiles || []).filter(
                (_, idx) => idx !== index,
              ),
              imagePreviews: (product.imagePreviews || []).filter(
                (_, idx) => idx !== index,
              ),
            }
          : product,
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([catRes, secRes, badRes]) => {
        const normalizedCategories = (catRes.data || [])
      ...
      .then((res) => {
        ...
        setSections((prev) =>
          uniqueNonEmpty([
            ...DEFAULT_SECTION_OPTIONS,
            ...prev,
            ...fetchedProducts.map((product) => product.section),
          ]),
        );
      })
  ...
  const categoryOptions = uniqueNonEmpty([
    ...DEFAULT_CATEGORY_OPTIONS,
    ...categories.map((category) => normalizeCategory(category)),
  ]);

  const sectionOptions = uniqueNonEmpty([
    ...DEFAULT_SECTION_OPTIONS,
    ...sections,
    ...products.map((product) => product.section),
  ]);
  ...
  const updateEditSubSection = (index, value) => {
    setEditForm((prev) => ({
      ...prev,
      subSections: prev.subSections.map((subSection, i) =>
        i === index ? value : subSection,
      ),
    }));
  };
  ...
  const removeEditSubSection = (index) => {
    setEditForm((prev) => {
      ...
      return {
        ...prev,
        subSections: next.length > 0 ? next : [""],
      };
    });
  };
  ...
  const updateEditProduct = (localId, field, value) => {
    setEditForm((prev) => ({
      ...prev,
      products: prev.products.map((product) =>
        product.localId === localId ? { ...product, [field]: value } : product,
      ),
    }));
  };
  ...
  const removeEditProduct = (localId) => {
    setEditForm((prev) => ({
      ...prev,
      products: prev.products.filter((product) => product.localId !== localId),
    }));
  };

  const saveEdit = async () => {
    const trimmedDescription = editForm.projectDescription.trim();
    const trimmedSubSections = uniqueNonEmpty(
      editForm.subSections.map((subSection) => subSection.trim()),
    );
    ...
    if (trimmedSubSections.length === 0) {
      setEditError("Add at least one sub-section.");
      return;
    }
    ...
    for (const product of editForm.products) {
      ...
      if (!DEFAULT_CATEGORY_OPTIONS.includes(product.category)) {
        setEditError("Category must be either Men or Women.");
        return;
      }
      if (!product.section) {
      ...
      if (Number.isNaN(discountedPrice) || discountedPrice < 0) {
        setEditError("Each product needs a valid discounted price.");
        return;
      }

      if (mrpValue !== null && (Number.isNaN(mrpValue) || mrpValue < 0)) {
        setEditError(
          "Original price (MRP) must be empty or a valid positive number.",
        );
        return;
      }

      if (mrpValue !== null && discountedPrice > mrpValue) {
        setEditError("Discounted price cannot be greater than original price.");
        return;
      }
    ...
    try {
      ...
      const deletedIds = originalEditProductIds.filter(
      ...
      for (const product of editForm.products) {
        ...
        if (product.id) {
          ...
          if (
            ...
            for (const existingImage of product.existingImages || []) {
              if (existingImage?.id) {
                await api.delete(`products/${product.id}/images/`, {
                  data: { image_id: existingImage.id },
                });
              }
            }
            const editImageData = new FormData();
            product.imageFiles.forEach((file) => {
              editImageData.append("images", file);
            });
            await api.post(`products/${product.id}/images/`, editImageData, {
      ...
      setSections((prev) => uniqueNonEmpty([...prev, ...trimmedSubSections]));
      closeEditModal();
  ...
  const handleAdd = async () => {
    ...
    if (!DEFAULT_CATEGORY_OPTIONS.includes(addForm.category)) {
      setAddError("Category must be either Men or Women.");
      return;
    }
    if (!addForm.section) {
    ...
    if (!addForm.description.trim()) {
      setAddError("Product description is required.");
      return;
    }
        ...
        <div className="admin-edit-modal">
          <div className="admin-edit-modal__group">
            <div className="admin-field">
              ...
              <textarea
                ...
                placeholder="Describe this project, collection, or featured story."
                rows={4}
              />
            </div>
          </div>
          ...
          <div className="admin-edit-modal__group">
            <div className="admin-edit-modal__heading-row">
              <h4 className="admin-edit-modal__heading">Sub-sections</h4>
              <button
                className="admin-action-btn admin-action-btn--edit"
                type="button"
                onClick={addEditSubSection}
              >
                + Add Sub-section
              </button>
            </div>
            <div className="admin-edit-subsections">
              {editForm.subSections.map((subSection, index) => (
                <div
                  key={`${index}-${subSection}`}
                  className="admin-edit-subsections__row"
                >
                  <input
                    className="admin-cell-input"
                    value={subSection}
                    onChange={(event) =>
                      updateEditSubSection(index, event.target.value)
                    }
                    placeholder={`Sub-section ${index + 1}`}
                  />
                  <button
                    type="button"
                    className="admin-action-btn admin-action-btn--delete"
                    onClick={() => removeEditSubSection(index)}
                    disabled={editForm.subSections.length === 1}
                  >
                    Remove
                  </button>
                </div>
                  ...
                  <div className="admin-add-form__grid admin-add-form__grid--edit-modal">
                    ...
                    <div className="admin-field">
                      ...
                      >
                        ...
                        {uniqueNonEmpty([
                          ...sectionOptions,
                          ...editForm.subSections.map((subSection) =>
                            subSection.trim(),
                          ),
                        ]).map((section) => (
                    ...
                    <div className="admin-field admin-field--image">
                      ...
                      <div className="admin-image-upload">
                        <input
                          ...
                          multiple
                          onChange={(event) =>
                            handleEditProductImageSelect(product.localId, event)
                          }
                        />
                        <label
                          className="admin-image-button"
                      ...
                      {Array.isArray(product.existingImages) &&
                        ...
                        (!Array.isArray(product.imagePreviews) ||
