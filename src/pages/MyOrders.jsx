...
export default function MyOrders() {
  ...
  // Refresh orders when page is visited
  useEffect(() => {
    fetchOrders();
  }, [fetchOrders]);
      ...
      <main className="orders-page">
        <section className="orders-hero">
          <div className="container orders-hero__inner">
            <span className="section-label">Orders</span>
            <h1>My Orders</h1>
            <p>Track your latest purchases and delivery progress in one place.</p>
          </div>
        </section>
        <section className="container orders-empty reveal">
          <h2>No orders yet</h2>
          <p>Once you place an order, tracking details will appear here.</p>
          <Link className="btn btn-primary" to="/">Start Shopping</Link>
        </section>
      </main>
    );
  }
    ...
    <main className="orders-page">
      <section className="orders-hero">
        <div className="container orders-hero__inner">
          <span className="section-label">Orders</span>
          <h1>My Orders</h1>
          <p>Track your order progress and estimated delivery date in real time.</p>
        </div>
      </section>

      <section className="container orders-list">
        {sortedOrders.map((order) => {
          const activeStage   = Number.isInteger(order.statusIndex) ? order.statusIndex : 0;
          const isHighlighted = selectedOrderId && String(selectedOrderId) === String(order.id);
          ...
          return (
            ...
            >
              <header className="order-card__header">
                <div>
                  <p className="order-card__meta">Order ID: {order.orderNumber}</p>
                  <h2>Placed on <span>{formatDate(order.createdAt)}</span></h2>
                </div>
                <span className="order-badge">{order.statusLabel || "Processing"}</span>
              </header>
              ...
              <div className="order-track">
                {ORDER_STAGES.map((stage, index) => {
                  const stateClass =
                    ...
                    index === activeStage ? "active" : "pending";
                  return (
                    <div key={stage} className={`order-track__step ${stateClass}`}>
